...
**Tips:**
- Prefix your question with `-a ` to query **all relevant hives** (slower,
  broader, and grounded in each hive's own evidence). Example: `-a explain me data mobility`
- Prefix with `-c ` to **clarify** an ambiguous question first — the assistant
  will ask 1-2 short questions, then merge your answer and proceed. Example: `-c explain me data mobility`
- Combine: `-c -a your question` to clarify then run all-hives mode.
...
def _sanitize_mermaid_blocks(text: str) -> str:
    """Fix common Mermaid syntax issues that break rendering.

    The LLM often emits node labels with unquoted parens / brackets like
    ``F[InitializeProtection(request)]`` which Mermaid can't parse.
    This wraps such labels in double quotes: ``F["InitializeProtection(request)"]``.

    Handles square ``[..]`` and rhombus ``{..}`` shapes (where 99% of LLM
    output errors occur). Other shapes are left alone.
    """
    import re as _re

    # Match either NodeId[label] or NodeId{label} where label has special chars.
    # Negative look-behind on `(` and look-ahead on `)` to avoid touching
    # cylinder ``[(..)]`` and stadium ``([..])`` shapes.
    pattern = _re.compile(
        r"(?<![(\[{])\b([A-Za-z_]\w*)([\[\{])([^\[\]{}\n]+?)([\]\}])(?![)\]}])"
    )

    def _wrap(m):
        node_id, opener, label, closer = m.group(1), m.group(2), m.group(3), m.group(4)
        # Opener and closer must match shape
        ...
        if stripped.startswith('"') and stripped.endswith('"'):
            return m.group(0)
        # Contains chars that need quoting?
        if any(c in label for c in "()[]<>|:"):
            safe = label.replace('"', '#quot;')
            return f'{node_id}{opener}"{safe}"{closer}'
        return m.group(0)

    def _fix_block(block: str) -> str:
        return pattern.sub(_wrap, block)

    fence_re = _re.compile(r"(```mermaid\s*\n)(.*?)(\n```)", _re.DOTALL)
    return fence_re.sub(lambda m: m.group(1) + _fix_block(m.group(2)) + m.group(3), text)
...
def _handle_save(cmd: str, brain) -> bool:
    ...
    # Sanitize all Mermaid fenced blocks so they render correctly
    full_md = _sanitize_mermaid_blocks("\n".join(md_lines))
...
def run_chat(config_path: Optional[str] = None):
    ...
    while True:
        try:
            ...
            # Parse leading flags: -a (all-hives), -c (clarifier)
            all_hives_mode = False
            clarify_mode = False
            tokens = user_input.split(maxsplit=0)  # placeholder; we parse manually
            # Strip flags from the front, in any order
            while True:
                if user_input.startswith("-a ") or user_input == "-a":
                    all_hives_mode = True
                    user_input = user_input[3:].strip() if len(user_input) > 2 else ""
                elif user_input.startswith("-c ") or user_input == "-c":
                    clarify_mode = True
                    user_input = user_input[3:].strip() if len(user_input) > 2 else ""
                else:
                    break
            ...
            if not user_input:
                console.print("[yellow]Usage: [-c] [-a] <your question>[/yellow]")
                continue

            # Optional clarification pass
            if clarify_mode and isinstance(brain, HiveRouter):
                with console.status("[bold cyan]Checking if I need to clarify...[/bold cyan]", spinner="dots"):
                    clar = brain.clarify_question(user_input)
                if clar.get("status") == "ask":
                    console.print("\n[cyan]I'd like to clarify a couple things first:[/cyan]")
                    answers: list[str] = []
                    for i, q in enumerate(clar["questions"], 1):
                        console.print(f"  [cyan]{i}. {q}[/cyan]")
                        try:
                            ans = prompt(f"   ?? Answer {i} > ", history=history, multiline=False).strip()
                        except (KeyboardInterrupt, EOFError):
                            ans = ""
                        if ans and ans.lower() not in ("skip", "s", ""):
                    ...
                    console.print("[dim](question is specific enough — proceeding)[/dim]")

            # Send to brain agent / hive router
            spinner_text = (
                "[bold green]Querying all relevant hives...[/bold green]"
                if all_hives_mode
                else "[bold green]Thinking...[/bold green]"
            )
            with console.status(spinner_text, spinner="dots"):
                if all_hives_mode and isinstance(brain, HiveRouter):
                    result = brain.chat_all_hives(user_input)
                else:
